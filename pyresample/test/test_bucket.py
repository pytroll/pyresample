...
class Test(unittest.TestCase):
    ...
    def test_init(self, get_indices, prj):
        ...
        self.assertTrue(hasattr(resampler, 'get_abs_min'))
    ...
    def _get_abs_min_result(self, data, **kwargs):
        """Compute the bucket abs min with kwargs and check that no dask computation is performed."""
        with dask.config.set(scheduler=CustomScheduler(max_computes=1)):
            result = self.resampler.get_abs_min(data, **kwargs)
        return result.compute()

    def test_get_abs_min(self):
        """Test abs min bucket resampling."""
        data = da.from_array(np.array([[-2, -11], [5, np.nan]]),
                             chunks=self.chunks)
        result = self._get_abs_min_result(data)
        # test multiple entries absolute minimum
        self.assertEqual(np.count_nonzero(result == -2), 1)
        # test single entry absolute minimum
        self.assertEqual(np.count_nonzero(result == 5), 1)
        # test that minimum of bucket with only nan is nan, and empty buckets are nan
        self.assertEqual(np.count_nonzero(~np.isnan(result)), 2)
